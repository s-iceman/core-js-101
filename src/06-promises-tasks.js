...
function willYouMarryMe(isPositiveAnswer) {
  switch (isPositiveAnswer) {
    case true: return Promise.resolve('Hooray!!! She said "Yes"!');
    case false: return Promise.resolve('Oh no, she said "No".');
    default: return Promise.reject(new Error('Wrong parameter is passed! Ask her again.'));
  }
}
...
function processAllPromises(array) {
  const res = [];
  Promise.allSettled(array).then((arr) => {
    arr.forEach((e) => (e.status === 'fulfilled' ? res.push(e.value) : res.push(e.reason)));
  });
